(function() {
  ...
  Asteroid.prototype.render = function() {
    ...
    if (this.topology[0] == 1) {
      stuff.push([this.position[0] + (this.x_bounds[1] - this.x_bounds[0]), this.position[1],
    ...
    for (var i = 0; i < stuff.length; i++) {
      ...
      if ((pos[0] >= this.x_bounds[0] - buff && pos[0] <= this.x_bounds[1] + buff) && (pos[1] >= this.y_bounds[0] - buff && pos[1] <= this.y_bounds[1] + buff)) {
        ...
        if (this.bonus > 0) {
          ctx.fillStyle = [null, 'red', 'yellow', 'green', 'blue'][this.bonus]
          ctx.font = (this.size * .9) + 'px vector_battleregular';
          ctx.fillText(this.shape, 0, 0);
          ctx.globalAlpha = 0.5;
          ctx.beginPath();
          ctx.ellipse(0, 0, this.type * 3, this.type * 3, 0, 0, 2 * Math.PI)
          ctx.fill();
          ctx.fillStyle = 'white';
          ctx.font = this.size + 'px vector_battleregular';
          ctx.globalAlpha = 1;
  ...
  Asteroid.prototype.move = function() {
    ...
    for (var dim = 0; dim < 2; dim++) {
      ...
      if (this.topology[dim] == 1 || this.topology[dim] == -1) {
        ...
        if ((this.position[dim] < bounds[0] && this.velocityVector[dim] < 0) || this.position[dim] > bounds[1] && this.velocityVector[dim] > 0) {

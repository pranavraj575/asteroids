(function() {
  ...
  Game.prototype.start = function() {
    ...
    this.requestAsteroids = true;
    this.last_render=-100;
    this.refresh(0);
  };

  Game.prototype.assignPoints = function(asteroid) {
    this.points += Math.round(Math.pow(2, 5 - asteroid.type));
  };
  ...
  Game.prototype.refresh = function(timestamp) {
    if(timestamp-this.last_render<this.spf){
  ...
  Game.prototype.showLives = function() {
    ...
    for (var i = 0; i < lives; i++) {
      ...
      if (i == lives - 1 && this.lifeblood) {
        ...
        ctx.globalAlpha = .69;
        ctx.fill();
      } else if (this.ship.shield_bash>0){
        ctx.fillStyle = 'green';
        ctx.globalAlpha = .69;
        ctx.fill();
      }
